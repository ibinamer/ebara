...
function parseWordPayload(payload: unknown): string | null {
  ...
  const word = payload.word.normalize("NFKC").trim().toLocaleLowerCase("en");
...
function normalizeDictionaryWord(value: unknown): string | null {
  const word = boundedString(value, MAX_WORD_LENGTH)?.toLocaleLowerCase("en");
  if (!word || !LATIN_WORD_PATTERN.test(word)) return null;
  return word;
}
...
async function fetchWiktionaryWikitext(
  ...
  try {
    response = await fetch(url, {
      ...
      headers: {
        Accept: "application/json",
        "Api-User-Agent": "VocabularyBox/1.0 (dictionary lookup)",
      },
      signal: controller.signal,
    });
  } catch (error) {
